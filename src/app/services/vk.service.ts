...
export class VkService {
  ...
  getGroupeIdUrl(url: string, count: number = 10) {
    ...
    return this.http.jsonp<any>(
      `https://api.vk.com/method/groups.getById?group_id=${screen_name}&access_token=${this.vk_access_token}&v=5.131`,
      'callback'
    );
  }
  getGroups() {
    this.getAccessToken();
    return this.http.jsonp<any>(
      `https://api.vk.com/method/groups.get?user_ids=${this.vk_user_id}&access_token=${this.vk_access_token}&extended=1&filter=moder&v=5.131`,
      'callback'
    );
  }
  ...
  getPostsGroup(group_id: number, count: number) {
    return this.http.jsonp<any>(
      `https://api.vk.com/method/wall.get?owner_id=${group_id}&access_token=${this.vk_access_token}&count=${count}&filter=all&extended=1&v=5.131`,
      'callback'
    );
  }
  ...
  getPostsMyPage(count: number) {
    return this.http.jsonp<any>(
      `https://api.vk.com/method/wall.get?owner_id=${this.vk_user_id}&access_token=${this.vk_access_token}&count=${count}&filter=all&extended=1&v=5.131`,
      'callback'
    );
  }
  ...
  isLikedUserVKEvent(group_id: number, post_id: number) {
    return this.http.jsonp<any>(
      `https://api.vk.com/method/likes.isLiked?user_id=${this.vk_user_id}&&owner_id=-${group_id}&&item_id=${post_id}&&type=post&access_token=${this.vk_access_token}&v=5.131`,
      'callback'
    );
  }
  ...
  serachCity(query: string, count: number = 100) {
    return this.http.jsonp<any>(
      `https://api.vk.com/method/database.getCities?access_token=${this.vk_service_key}&country_id=1&q=${query}&need_all=1&count=${count}&v=5.131`,
      'callback'
    );
  }
}
