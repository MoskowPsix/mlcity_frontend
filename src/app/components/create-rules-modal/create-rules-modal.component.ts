...
export class CreateRulesModalComponent implements OnInit {
  ...
  constructor(
    private rulesModalCheckService:RulesModalCheckService
  ) {
...
closeModalFunc(){
 ...
 let accept = {
  ...
  this.rulesModalCheckService.setAgreements(accept).pipe().subscribe((res)=>{
...
userReadRulesFnc(event:any){
  ...
  ngOnInit() {
    ...
    this.rulesModalCheckService.getAgreements().pipe().subscribe((res)=>{
