            ...
            this.locationService.getLocationByCoords([Number(lastMapLatitude), Number(lastMapLongitude)]).pipe(
                tap((res: any) => {
                  ...
                  locationParrent = res.location.location_parent.name
                    places.forEach((place: any) => {
                        if (place.location.name && (place.location.name === locationName || place.location.name === res.location.location_parent.name)) {
                ...
                next: () => {
                    if(locationPlaces.length != 0){
                     ...
                     resolve({seance:minSeance.seance,place:minSeance.place, loacation_name:locationName, location_parent:locationParrent, });
                    }else{
