  ...
  onMapClick(e: YaEvent<ymaps.Map>): void {
    ...
    if (this.typeSelected == 1) {
       ...
       if (!this.placemarkStart) {
        ...
        this.pol.push([event.get('coords')[0].toPrecision(6), event.get('coords')[1].toPrecision(6)])
      ...
      if (!this.placemarkStart) {
        ...
        this.createAutoRouteMap(this.placemarkStart, this.placemarkStart.geometry?.getCoordinates()!,this.placemarkStop.geometry?.getCoordinates()!)
      } 
...
this.pol.forEach(el=>{console.log(el)})
this.map.target.geoObjects.removeAll()
    ...
    //создаем проходные точки
     ...
     multiRoute.editor.start({addMidPoints: true, addWayPoints: false, removeWayPoints: true,});
    this.map.target.geoObjects.add(multiRoute);
  }

//Настройки авто маршрута
createAutoRouteMap(start: ymaps.Placemark, coordsStart: number[], coordsStop: number[]){
  var multiRoute = new ymaps.multiRouter.MultiRoute({
    referencePoints: [
